<?php
// app/controllers/Linked_profilesController.php
 ...
 * they manage.
 */
class Linked_profilesController extends Controller {
    ...
    public function index() {
        ...
        // If the user canceled deletion of a linked profile, show a notification
        ...
        if (is_post()) {
            ...
            // or the OTP verification (step 2) by checking the posted parameters.
            $phone = $_POST['phone'] ?? null;
            $otp   = $_POST['otp'] ?? null;
            if ($phone !== null) {
                // Only proceed to OTP step if phone submission is successful
            ...
            // When navigating or reloading, preserve the OTP step if there is a pending
            // linking session (phone and profile ID stored). This allows users to re‑enter
            // the OTP or resend it without having to input the phone number again.
        ...
        // Preserve submitted phone/otp values so that input boxes are not cleared on errors
    ...
    /**
     * Resend a new OTP for a pending link request.
     *
     * This method regenerates the OTP, resets the expiry and attempts,
     * and flashes the new code to the user. If there is no pending
     * request, the user is redirected back with an error message.
     */
    public function resend() {
        require_role(['customer']);
        // Ensure there is a pending link in session
        if (!isset($_SESSION['link_phone']) || !isset($_SESSION['link_profile_id'])) {
            flash('error', 'Không có yêu cầu liên kết hồ sơ đang chờ.');
            return redirect('linked_profiles');
        }
        // Generate a fresh OTP and update session variables
        // Always use a fixed OTP for demo purposes (matching registration flow)
        $otp = '000000';
        $_SESSION['link_otp']          = $otp;
        $_SESSION['link_otp_expiry']   = time() + 60;
        $_SESSION['link_otp_attempts'] = 0;
        // Notify the user that a new verification code has been sent without revealing the code
        flash('info', 'Mã xác thực mới đã được gửi.');
        return redirect('linked_profiles');
    }

    /**
     * Cancel the current linking process and return to the phone entry step.
     *
     * This clears any stored phone, profile ID and OTP from the session,
     * stops any ongoing timers and redirects back to the main form where
     * the user can enter a new phone number.
     */
    public function reset() {
        require_role(['customer']);
        // Clear any linking session variables
        $this->clearLinkSession();
        flash('info', 'Bạn có thể nhập lại số điện thoại để liên kết hồ sơ.');
        return redirect('linked_profiles');
    }
    ...
    private function handlePhoneSubmission(string $phone, Database $db, string $userId): bool {
        ...
        // Generate OTP for demo: always use 000000 like registration flow
        $otp = '000000';
        // Store linking metadata in session
        $_SESSION['link_phone']        = $phone;
        $_SESSION['link_profile_id']   = $maKh;
        $_SESSION['link_otp']          = $otp;
        $_SESSION['link_otp_expiry']   = time() + 60; // Valid for 1 minute
        $_SESSION['link_otp_attempts'] = 0;
        // Inform the user that a verification code has been sent without revealing the code
